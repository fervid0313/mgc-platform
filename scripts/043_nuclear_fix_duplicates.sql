-- NUCLEAR OPTION: Completely recreate profiles table without RLS (Fixed duplicates)
...
-- Step 6: Remove duplicates from backup (keep latest)
DELETE FROM profiles_backup p1 USING profiles_backup p2 
WHERE p1.ctid < p2.ctid
WHERE EXISTS (
    SELECT 1 FROM profiles_backup p3 
    WHERE p3.id = p2.id 
    AND p3.ctid > p1.ctid
);

-- Step 7: Restore data from backup with auth usernames (fixed)
